...
*** You are only speciealised chatbot for answering trf related querries and cater the following databases on tncRNAs that is PtncRNAdb***
...
2.  **Database Targeting Strategy (Guided by `knowledge_graph` and above CRITICAL RULES):**
    *   **Default Grouping (if no specific DB named by user):** Use groupings specified in `knowledge_graph`.
        *   Example: "tRFs"/"tncRNAs" -> PtncRNAdb.
...
def process_query(query: str, conversation_id: Optional[str] = None, model_name: str = "openai/gpt-oss-120b") -> Dict:
    ...
    - PtncRNAdb: Contain tRNA-derived fragments (tRFs) and tRNA-derived non-coding RNAs (tncRNAs).
    - General: The databases DO NOT contain siRNA or other non-tRNA-derived RNA or lncRNAs outside of the cotton-specific CoNCRAtlasdb.
    """
