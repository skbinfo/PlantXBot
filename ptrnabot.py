...
def generate_csv(results: List[Dict], query_context: str, full_results: Optional[List[Dict]] = None) -> str:
    """
    Generate CSV from either provided results or full results from stats.
    """
    if full_results and len(full_results) > len(results):
        # Use full results for download if available
        csv_data = full_results
        logger.info(f"Generating CSV from full dataset: {len(full_results)} rows")
    else:
        # Fallback to provided results
        csv_data = results
        logger.info(f"Generating CSV from provided results: {len(results)} rows")
    ...
    try:
        ...
        logger.info(f"Generated CSV: {filepath} ({len(csv_data)} rows), URL: {download_url}")
        return download_url
    except Exception as e:
...
def generate_statistics_from_results(results: List[Dict], query_context: str = "query") -> Dict:
    ...
    if not results:
        return {"count": 0, "column_stats": {}, "preview": [], "full_results_for_download": [], "stats_based_on_sample": False}

    original_count = len(results)
    # Preview for UI display (limited to 10 rows)
    preview_for_display = results[:DISPLAY_ROW_LIMIT]
    # Keep full results for download
    full_results_for_download = results.copy()
    stats_based_on_sample = False
    data_for_stats = results

    if original_count > MAX_ROWS_FOR_PANDAS_STATS:
        logger.warning(f"[{request_id_prefix}] Large result set ({original_count} rows). Sampling {MAX_ROWS_FOR_PANDAS_STATS} rows for statistics.")
        data_for_stats = random.sample(results, MAX_ROWS_FOR_PANDAS_STATS)
        stats_based_on_sample = True
        # Still keep full results for download (if reasonable size)
        if original_count > 100000:  # Safety limit for very large downloads
            logger.warning(f"[{request_id_prefix}] Very large dataset ({original_count} rows). Full download will be truncated to 100,000 rows.")
            full_results_for_download = random.sample(results, 100000)
    ...
    return {
        ...
        "preview": preview_for_display,  # Only 10 rows for UI preview
        "full_results_for_download": full_results_for_download,  # Full data for CSV download
...
*** You are a specialized chatbot for answering queries related to tRNA catering to the following database PtRNAdb. ***
...
SQL_PLAN_PROMPT_TEMPLATE = PromptTemplate(
    ...
    template="""
You are an AI assistant designed to analyze user queries and plan data retrieval or information provision for plant non-coding RNA (ncRNA) database. Your task is to generate precise, SQLite-compatible SQL queries or metadata responses based on the provided database schemas and knowledge graph.

*** You are a specialized chatbot for answering queries related to tRNA catering to the databases specified in the `database_table_mapping_info`. ***
...
- Ensure all queries are safe (SELECT or WITH statements only). **DO NOT use LIMIT clauses in SQL queries - the Python layer handles preview limiting.**
...
- **PtRNAdb (PtRNAdb):** Contains plant tRNA gene data across many species. Key columns include: `PtRNAdb ID`, `Plant Source`, `Genome level`, `tRNA ID`, `tRNA locus`, `Isoacceptor`, `Isodecoder`, `tRNA Sequence`, `Infernal Score`.
  - Notes: Multi-species data in `Plant Source`. Scores are REAL and can be used directly for numeric operations.

**CRITICAL DATABASE-SPECIFIC RULES & DATA SEMANTICS:**
- **Species and Tissue Handling:**
  - Species: `PtRNAdb` has a `Plant Source` column. Use exact matching or LIKE with wildcards for species names.
- **Numeric Columns:** In `PtRNAdb`, score columns (`Infernal Score`, `HMM Score`, `Secondary structure Score`) are REAL. Use them directly for numeric comparisons.
...
3. **Response Strategy & SQL Planning:**
   ...
   - **DATA_RETRIEVAL:**
     ...
     - **Simple Listings (e.g., "List tRNAs in rice"):**
       ...
       - `sql`: `SELECT "PtRNAdb ID", "tRNA ID", "Isoacceptor", "Plant Source" FROM "PtRNAdb" WHERE "Plant Source" LIKE '%Oryza sativa%';`.
     - **Entity Lookups (e.g., "Details for tRNA ID X"):**
       - `purpose_type`: `entity_lookup_detail`.
       - `sql`: `SELECT * FROM "PtRNAdb" WHERE "tRNA ID" = 'X';`.
     - **Species-specific queries (e.g., "All tRNA genes for Actinidia chinensis"):**
       - `purpose_type`: `data_preview`.
       - `sql`: `SELECT * FROM "PtRNAdb" WHERE "Plant Source" = 'Actinidia chinensis';`.
...
4. **SQL Best Practices:**
   ...
   - **IMPORTANT: Do not use LIMIT clauses in any SQL query. The application handles row limiting for UI preview separately.**
   - Use exact column names from the schema. For PtRNAdb: "PtRNAdb ID", "Plant Source", "tRNA ID", "Isoacceptor", etc.
    ...
    if query_type_from_stage1 == "metadata_answered_from_blob":
        ...
        if direct_answer_from_metadata and direct_answer_from_metadata.strip():
            ...
            concept_keywords_map = {
                "tRNA": ["tRNA (transferRNA)", "PtRNA Database"]
            ...
            DATABASE_METADATA_KNOWLEDGE_BLOB = """**PtRNAdb Database(PtRNAdb):** PtRNAdb is a plant exclusive database containing information of tRNA genes, identified in 106 nuclear 89 plastidial and 38 mitochondrial genomes.
"""
            ...
            try:
                ...
                if isinstance(actual_results, str):
                    ...
                    if actual_results:
                        ...
                        # Pass full_results_for_download from stats
                        full_results_for_csv = full_stats_obj.get("full_results_for_download", actual_results)
                        csv_url = generate_csv(actual_results, f"query_{i+1}_{csv_filename_context.replace(' ', '_')}_{purpose_type}", full_results=full_results_for_csv)
                        if csv_url:
                           current_query_display_details["download_url"] = csv_url
                           # Add info about download size
                           current_query_display_details["download_info"] = {
                           "total_rows_for_download": len(full_results_for_csv),
                           "preview_rows": len(full_stats_obj["preview"])
                              }
