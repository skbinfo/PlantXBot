                ...
                'tables': {
                    'AtFusionDB': {
                        'columns': [
                            ...
                            {'name': 'gene1', 'sqlite_type': 'TEXT', 'semantic_type': 'gene_identifier', 'description': 'First gene involved in the fusion (AGI ID).'},
                            {'name': 'gene2', 'sqlite_type': 'TEXT', 'semantic_type': 'gene_identifier', 'description': 'Second gene involved in the fusion (AGI ID).'},
                            ...
                            {'name': 'sequence', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'Full nucleotide sequence, may contain "unable to predict".'}
                        ],
                        'description': 'Stores basic fusion transcripts in Arabidopsis thaliana with gene pairs (AGI IDs), tissue, and sequence data. It does not contain functional annotations or expression scores.',
                        'primary_keys': ['identifier'],
                        'common_joins': {'PFusionDB': "On `gene1`, `gene2` for Arabidopsis-specific fusions."},
                        'notes': ["Data specific to Arabidopsis thaliana. No 'species' or 'organism' column exists. Lacks detailed functional gene annotations, protein names, and expression scores."]
                    },
                    'PFusionDB': {
                        'columns': [
...
INTENT_CLASSIFICATION_PROMPT_TEMPLATE = PromptTemplate(
    ...
    template="""
You are an expert bioinformatics assistant specialized in classifying user queries for plant genomics databases focused on fusion transcripts (PFusionDB and AtFusionDB). Your goal is to accurately classify the intent while ensuring queries are within scope.

Step-by-Step Reasoning (Chain-of-Thought):
1. Read the user query and conversation summary.
2. Identify key elements: Is it general (e.g., about capabilities) or specific (e.g., gene ID, function)?
3. Match to categories: Check for metadata (general facts), data retrieval (specific records), or out-of-scope.
4. Use examples below to guide classification.
5. Output ONLY the classification string.

****Clarification on "what data" queries:****
- General (no topic): "METADATA_DIRECT_ANSWER_PREFERRED" (e.g., "what data do you have?" → answer from knowledge base, no SQL).
- Specific topic: "DATA_RETRIEVAL" (e.g., "what data on AT1G28520?" → preview records).

Few-Shot Examples:
- Query: "what data do you have?" → Classification: METADATA_DIRECT_ANSWER_PREFERRED
- Query: "show me data about kinase activity" → Classification: DATA_RETRIEVAL
- Query: "how many species in PFusionDB?" → Classification: METADATA_SQL_FALLBACK_PREFERRED
- Query: "what is the time?" → Classification: OUT_OF_SCOPE

User Query: {user_query}
Conversation Summary: {conversation_summary}
Available Databases: {available_databases_summary}
Database Content Summary: {database_content_summary}

Classify into ONE category:
1. "METADATA_DIRECT_ANSWER_PREFERRED": General knowledge about databases/concepts (no previews/SQL).
2. "METADATA_SQL_FALLBACK_PREFERRED": Metadata needing simple SQL (e.g., count species).
3. "DATA_RETRIEVAL": Specific data fetch/preview/comparison.
4. "AMBIGUOUS": Unclear but domain-related.
5. "TYPO": Likely typo in domain query.
6. "GENERAL_CONVERSATION": Greeting/closing.
7. "OUT_OF_SCOPE": Unrelated to fusion transcripts.

Output ONLY the classification string (e.g., "DATA_RETRIEVAL").
Classification:
"""
)
# knowledge text for context (e.g., "how many species are in PbtRF?", "list tissue types in AtFusionDB?", "what peptide functions are listed in plantpepdb?").
#3.  "DATA_RETRIEVAL": ***Any Query starting with ("what is the", "which","fetch","tell me about","what can be", "give me" etc.)***, Query asks to find specific data records, perform calculations on data, or compare data sets *within the described scope of the databases* (e.g., "find genes in Arabidopsis involved in fusions", "list tRFs with expression > 2 in PbtRF").**Crucially, this also includes vague follow-up commands (like 'ok do it', 'show me', 'run that', 'yes please') that directly relate to a data action proposed or described by the bot in the `conversation_summary`.** Use the context to see if the user is confirming a previous action.
...
*** You are only specialised chatbot for answering fusion transcript related queries and cater the following databases on fusion transcripts that are AtFusionDB Database and
 PFusionDB Database. ***
...
****Additional Rule for Previews: For general 'data_preview' requests (e.g., "what data do you have on fusions?"), use a small LIMIT (e.g., 5) to avoid large results. For specific lookups, use no or higher LIMIT based on query.****
...
2.  **GENE ID QUERIES (e.g., "AT1G28520"):** This is the most important rule.
    ...
    * **QUERIES:** The `queries` array MUST contain TWO separate query objects, one for each database.
        * **Query 1 (AtFusionDB):**
            * `sql`: `SELECT * FROM "AtFusionDB" WHERE "gene1" = 'THE_GENE_ID' OR "gene2" = 'THE_GENE_ID';`
            * `target_table`: "AtFusionDB"
        * **Query 2 (PFusionDB):**
...
4.  **EXPRESSION-BASED QUERIES:** This data is ONLY in `PFusionDB`.
...
{{
  ...
  "queries": [
    {{
      "sql": "SELECT * FROM \\"AtFusionDB\\" WHERE \\"gene1\\" = 'AT1G28520' OR \\"gene2\\" = 'AT1G28520';",
      "target_table": "AtFusionDB",
      "database_conceptual_name": "AtFusionDB Database",
      "description": "Checking for fusions involving AT1G28520 in AtFusionDB.",
...
def query_endpoint():
    try:
        ...
        response_data = process_query(user_query, conversation_id=conversation_id, model_name=model_name)
