...
DATABASE_MAPPING = {
    "athisomir": "AthisoMir Database",
    "anninter2": "AnnInter2 Database",
    "linc_data": "Alnc Database",
    "PtRNAdb": "PtRNAdb",  # <-- NEW: Added PtRNAdb mapping
}
# Rate Limiting
limiter = Limiter(
    app=app,
        ...
        self.schema = {
            'all_databases.db': {
                'tables': {
                    'athisomir': {
                        'columns': [
                            {'name': 'Isomir_ID', 'sqlite_type': 'TEXT', 'semantic_type': 'primary_identifier', 'description': 'Unique identifier for the isomiR.'},
                            {'name': 'Sample', 'sqlite_type': 'TEXT', 'semantic_type': 'sample_id', 'description': 'Sample identifier.'},
                            {'name': 'Isomir_Type', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'Type of isomiR (e.g., 5p, 3p).'},
                            {'name': 'Sequence', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'miRNA/isomiR sequence.'},
                            {'name': 'RPM', 'sqlite_type': 'INT', 'semantic_type': 'expression_value', 'description': 'Reads Per Million for expression.'},
                            {'name': 'Valid', 'sqlite_type': 'TEXT', 'semantic_type': 'flag', 'description': 'Validation status of the isomiR.'},
                            {'name': 'Name', 'sqlite_type': 'TEXT', 'semantic_type': 'name', 'description': 'Name of the isomiR.'},
                            {'name': 'Alias', 'sqlite_type': 'TEXT', 'semantic_type': 'alias', 'description': 'Alternative name for the isomiR.'},
                            {'name': 'Parent', 'sqlite_type': 'TEXT', 'semantic_type': 'parent_identifier', 'description': 'Parent miRNA identifier.'},
                            {'name': 'Family', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'miRNA family.'},
                            {'name': 'Variant', 'sqlite_type': 'TEXT', 'semantic_type': 'variant_type', 'description': 'Variant type of the isomiR.'},
                            {'name': 'Cigar', 'sqlite_type': 'TEXT', 'semantic_type': 'sequence_feature', 'description': 'CIGAR string for alignment.'},
                            {'name': 'Hits', 'sqlite_type': 'INT', 'semantic_type': 'count', 'description': 'Number of alignment hits.'},
                            {'name': 'Chr', 'sqlite_type': 'INT', 'semantic_type': 'chromosome', 'description': 'Chromosome number.'},
                            {'name': 'Start', 'sqlite_type': 'INT', 'semantic_type': 'genomic_position', 'description': 'Start position in genome.'},
                            {'name': 'End', 'sqlite_type': 'INT', 'semantic_type': 'genomic_position', 'description': 'End position in genome.'},
                            {'name': 'Strand', 'sqlite_type': 'TEXT', 'semantic_type': 'strand', 'description': 'Genomic strand (+ or -).'},
                            {'name': 'GFF', 'sqlite_type': 'INT', 'semantic_type': 'metadata', 'description': 'GFF annotation identifier.'},
                            {'name': 'RepeatMask', 'sqlite_type': 'TEXT', 'semantic_type': 'metadata', 'description': 'Repeat masker annotation.'},
                            {'name': 'Hairpin', 'sqlite_type': 'TEXT', 'semantic_type': 'sequence_feature', 'description': 'Hairpin structure data.'},
                            {'name': 'Hairpin_Location', 'sqlite_type': 'TEXT', 'semantic_type': 'genomic_position', 'description': 'Location of hairpin structure.'},
                            {'name': 'Hairpin_Position', 'sqlite_type': 'TEXT', 'semantic_type': 'genomic_position', 'description': 'Position within hairpin.'},
                            {'name': 'Mutation', 'sqlite_type': 'TEXT', 'semantic_type': 'sequence_feature', 'description': 'Mutation details.'},
                            {'name': 'MatureMeta', 'sqlite_type': 'TEXT', 'semantic_type': 'metadata', 'description': 'Mature miRNA metadata.'},
                            {'name': 'MatureMeta_Offset', 'sqlite_type': 'TEXT', 'semantic_type': 'metadata', 'description': 'Offset in mature miRNA metadata.'}
                        ],
                        'description': 'IsomiR data for Arabidopsis thaliana, focusing on miRNA variants and their expression.',
                        'primary_keys': ['Isomir_ID'],
                        'common_joins': {
                            'anninter2': "On `Sequence` or `Parent` with `interactor1_seq` or `interactor2_seq`.",
                            'linc_data': "On `Sequence` or genomic coordinates (`Chr`, `Start`, `End`)."
                        },
                        'notes': ["Specific to Arabidopsis thaliana. Use `RPM` for expression analysis."]
                    },
                    'anninter2': {
                        'columns': [
                            {'name': 'pair_id', 'sqlite_type': 'TEXT', 'semantic_type': 'primary_identifier', 'description': 'Unique identifier for the RNA interaction pair.'},
                            {'name': 'anninter_id', 'sqlite_type': 'TEXT', 'semantic_type': 'secondary_identifier', 'description': 'Annotation interaction ID.'},
                            {'name': 'interactor1', 'sqlite_type': 'TEXT', 'semantic_type': 'identifier', 'description': 'First RNA interactor ID.'},
                            {'name': 'rna_type1', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'Type of first RNA (e.g., miRNA, lncRNA).'},
                            {'name': 'coordinate1', 'sqlite_type': 'TEXT', 'semantic_type': 'genomic_position', 'description': 'Genomic coordinates of interactor1.'},
                            {'name': 'interactor2', 'sqlite_type': 'TEXT', 'semantic_type': 'identifier', 'description': 'Second RNA interactor ID.'},
                            {'name': 'rna_type2', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'Type of second RNA (e.g., miRNA, lncRNA).'},
                            {'name': 'coordinate2', 'sqlite_type': 'TEXT', 'semantic_type': 'genomic_position', 'description': 'Genomic coordinates of interactor2.'},
                            {'name': 'splice_site', 'sqlite_type': 'TEXT', 'semantic_type': 'sequence_feature', 'description': 'Splice site information.'},
                            {'name': 'interaction_site', 'sqlite_type': 'TEXT', 'semantic_type': 'sequence_feature', 'description': 'Interaction site details.'},
                            {'name': 'interactor1_seq', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'Sequence of interactor1.'},
                            {'name': 'interactor2_seq', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'Sequence of interactor2.'},
                            {'name': 'allenscore', 'sqlite_type': 'TEXT', 'semantic_type': 'score', 'description': 'Allen score for interaction strength.'},
                            {'name': 'category', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'Interaction category.'},
                            {'name': 'p_value', 'sqlite_type': 'TEXT', 'semantic_type': 'statistical_value', 'description': 'P-value for interaction significance.'},
                            {'name': 'expectation_value', 'sqlite_type': 'TEXT', 'semantic_type': 'statistical_value', 'description': 'Expectation value for interaction.'},
                            {'name': 'upe', 'sqlite_type': 'TEXT', 'semantic_type': 'energy_value', 'description': 'Unpaired energy.'},
                            {'name': 'Inhibition', 'sqlite_type': 'TEXT', 'semantic_type': 'flag', 'description': 'Inhibition status of interaction.'},
                            {'name': 'mfe_rnahybrid', 'sqlite_type': 'TEXT', 'semantic_type': 'energy_value', 'description': 'Minimum free energy (RNAhybrid).'},
                            {'name': 'mfe_intarna', 'sqlite_type': 'TEXT', 'semantic_type': 'energy_value', 'description': 'Minimum free energy (IntaRNA).'},
                            {'name': 'tool', 'sqlite_type': 'TEXT', 'semantic_type': 'metadata', 'description': 'Tool used for prediction.'},
                            {'name': 'demo', 'sqlite_type': 'TEXT', 'semantic_type': 'metadata', 'description': 'Demonstration metadata.'},
                            {'name': 'sample_id', 'sqlite_type': 'TEXT', 'semantic_type': 'sample_id', 'description': 'Sample identifier for interaction.'}
                        ],
                        'description': 'RNA-RNA interaction data, including miRNA and lncRNA interactions.',
                        'primary_keys': ['pair_id'],
                        'common_joins': {
                            'athisomir': "On `interactor1_seq` or `interactor2_seq` with `Sequence` or `Parent`.",
                            'linc_data': "On `interactor1_seq` or `interactor2_seq` with `Sequence`."
                        },
                        'notes': ["Focuses on RNA interactions. Use `p_value` or `allenscore` for significance."]
                    },
                    'linc_data': {
                        'columns': [
                    ...
                    # <-- NEW: Added PtRNAdb schema definition -->
                    'PtRNAdb': {
    'columns': [
        {'name': 'PtRNAdb ID', 'sqlite_type': 'TEXT', 'semantic_type': 'primary_identifier', 'description': 'Unique identifier for the tRNA record.'},
        {'name': 'Plant Source', 'sqlite_type': 'TEXT', 'semantic_type': 'organism_name', 'description': 'The plant species the tRNA is from.'},
        {'name': 'Genome level', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'The genomic origin (e.g., nuclear, plastidial, mitochondrial).'},
        {'name': 'tRNA ID', 'sqlite_type': 'TEXT', 'semantic_type': 'secondary_identifier', 'description': 'Identifier for the specific tRNA.'},
        {'name': 'tRNA locus', 'sqlite_type': 'TEXT', 'semantic_type': 'genomic_position', 'description': 'Genomic locus of the tRNA.'},
        {'name': 'Isoacceptor', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'The isoacceptor family of the tRNA (e.g., tRNA-Ala).'},
        {'name': 'Isodecoder', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'The isodecoder of the tRNA based on anticodon.'},
        {'name': 'tRNA Sequence', 'sqlite_type': 'REAL', 'semantic_type': 'biological_sequence', 'description': 'The full sequence of the tRNA gene.'},
        {'name': 'tRNA secondary structure', 'sqlite_type': 'TEXT', 'semantic_type': 'sequence_feature', 'description': 'Predicted secondary structure in dot-bracket notation.'},
        {'name': 'Upstream sequence (upto 100nt)', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'Genomic sequence upstream of the tRNA gene.'},
        {'name': 'Downstream sequence (upto 100nt)', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'Genomic sequence downstream of the tRNA gene.'},
        {'name': 'Mature tRNA', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'Details about the mature tRNA.'},
        {'name': 'Intron Sequence', 'sqlite_type': 'REAL', 'semantic_type': 'biological_sequence', 'description': 'Sequence of any intron within the tRNA gene.'},
        {'name': 'Intron Position', 'sqlite_type': 'REAL', 'semantic_type': 'genomic_position', 'description': 'Position of the intron.'},
        {'name': 'Infernal Score', 'sqlite_type': 'REAL', 'semantic_type': 'score', 'description': 'Score from the Infernal tool for covariance model search.'},
        {'name': 'HMM Score', 'sqlite_type': 'REAL', 'semantic_type': 'score', 'description': 'Hidden Markov Model score for sequence alignment.'},
        {'name': 'Secondary structure Score', 'sqlite_type': 'REAL', 'semantic_type': 'score', 'description': 'Score related to the secondary structure prediction.'}
    ],
    'description': 'A plant-exclusive tRNA gene database from 106 nuclear, 89 plastidial, and 38 mitochondrial genomes.',
    'primary_keys': ['PtRNAdb ID'],
    'common_joins': {},
    'notes': ["Focuses on tRNA genes across many plant species. Score columns and some sequence/position columns are stored as REAL."]
                  }
                }
            }
        }
...
**General Concepts:**
- **miRNA (microRNA):** Small non-coding RNAs (~22 nt) that regulate gene expression, with isomiRs being sequence variants.
- **lncRNA (long non-coding RNA):** Non-coding RNAs >200 nt, involved in regulatory processes.
- **tRNA (transfer RNA):** Small RNA molecules that help decode mRNA sequences into proteins.
- **RNA Interactions:** Interactions between RNAs (e.g., miRNA-lncRNA, miRNA-mRNA,miRNA-siRNA) predicted by tools like RNAhybrid or IntaRNA.

**Database Summaries:**
*   **AthisoMir Database (athisomir):** Arabidopsis thaliana isomiR (miRNA variant) data. Key data: `Sequence`, `Isomir_Type` (e.g., 5p, 3p), `RPM` (expression), `Parent` (parent miRNA), genomic coordinates (`Chr`, `Start`, `End`). Specific to Arabidopsis only.
*   **AnnInter Database (anninter2):** RNA-RNA interaction data, including miRNA and lncRNA interactions. Key data: `interactor1_seq`, `interactor2_seq`, `rna_type1`, `rna_type2`, `p_value`, `allenscore` (interaction strength), `mfe_rnahybrid` (energy). Specific to Arabidopsis thaliana.
...
*   **PtRNAdb (PtRNAdb):** A plant-exclusive tRNA gene database from 106 nuclear, 89 plastidial, and 38 mitochondrial genomes. Key data: `Plant Source` (species), `tRNA ID`, `Isoacceptor`, `Isodecoder`, `tRNA Sequence`, and various scores. Focuses on tRNA gene characteristics.
"""
...
*** You are a specialized chatbot for answering queries related to miRNA, lncRNA, tRNA, and RNA interactions, catering to the following databases: AthisoMir Database, AnnInter2 Database, Alnc Database, and PtRNAdb. ***
...
1. "METADATA_DIRECT_ANSWER_PREFERRED": The query asks for general knowledge or facts ABOUT the databases (e.g., "What is AthisoMir?", "What are tRNAs?", "Do you have data on RNA interactions?", "What kind of data is in Alnc Database?"). These should be answered from a knowledge base.
2. "METADATA_SQL_FALLBACK_PREFERRED": Query asks for metadata *about the data within the databases* that might require a simple SQL query if not directly in a knowledge text, but still benefits from the knowledge text for context (e.g., "How many species are in PtRNAdb?", "List tissue types in Alnc Database?", "What miRNA types are in AthisoMir?").
...
# --- CORRECTED: Updated SQL Planning Prompt for ncRNA Databases ---
...
*** You are a specialized chatbot for answering queries related to miRNA, isomiR, lncRNA, tRNA, and RNA interactions, catering to the databases: AthisoMir Database (athisomir), AnnInter2 Database (anninter2), Alnc Database (linc_data), and PtRNAdb (ptRNAdb). ***
...
- Knowledge Graph: {knowledge_graph}
    (Contains:
        - Table purposes and conceptual names (use `database_table_mapping_info` for output).
        - Mappings of semantic concepts (e.g., 'species', 'tissue', 'expression') to column names.
...
- UI Preview Row Limit: {DISPLAY_ROW_LIMIT} (UI hint ONLY, NOT for SQL `LIMIT` unless specified for previews)

**Database Overview (Derived from Schemas):**
- **AthisoMir Database (athisomir):** Contains Arabidopsis thaliana miRNA/isomiR data. Key columns: `"Isomir_ID"`, `"Sample"`, `"Isomir_Type"`, `"Sequence"`, `"RPM"`, `"Valid"`, `"Name"`, `"Alias"`, `"Parent"`, `"Family"`, `"Variant"`, `"Cigar"`, `"Chr"`, `"Start"`, `"End"`, `"Strand"`, `"Repeat OverlapHairpin_ID"`, `"Hairpin_Location"`, `"IsomiR Position within Hairpin"`, `"3’ Non-templated addition"`, `"MatureMeta"`, `"MatureMeta_Offset"`, `"Target_Acc."`, `"Expectation"`, `"UPE"`, `"miRNA_start"`, `"miRNA_end"`, `"Target_start"`, `"Target_end"`, `"miRNA_aligned_fragment"`, `"Target_aligned_fragment"`, `"Inhibition"`, `"Target_Desc."`, `"Tissues"`, `"Stage"`, `"Conditions"`, `"Genotype"`.
  - Notes: Arabidopsis-specific (no species column). `"RPM"` (TEXT, CAST to REAL for numerics) for expression. Use `"Tissues"`, `"Conditions"` for filtering.
- **AnnInter2 Database (anninter2):** Contains RNA-RNA interaction data (e.g., miRNA-lncRNA). Key columns: `"pair_id"`, `"anninter_id"`, `"interactor1"`, `"rna_type1"`, `"coordinate1"`, `"interactor2"`, `"rna_type2"`, `"coordinate2"`, `"splice_site"`, `"interaction_site"`, `"interactor1_seq"`, `"interactor2_seq"`, `"allenscore"`, `"category"`, `"p_value"`, `"expectation_value"`, `"upe"`, `"Inhibition"`, `"mfe_rnahybrid"`, `"mfe_intarna"`, `"tool"`, `"demo"`, `"sample_id"`.
  - Notes: Arabidopsis-specific (no species column). `"p_value"`, `"allenscore"`, `"expectation_value"` are TEXT; CAST to REAL for comparisons. `"rna_type1"`, `"rna_type2"` indicate molecule types.
- **Alnc Database (linc_data):** Contains long non-coding RNA (lncRNA) data across multiple species. Key columns: `"AlnC_ID"`, `"Sample_code"`, `"Species"`, `"Tissue"`, `"Source_transcript_ID"`, `"Coding_probability"`, `"Non_coding_probability"`, `"Length"`, `"GC_content"`, `"NCBI_link"`, `"Sequence"`.
  - Notes: Multi-species (`"Species"`). Use `"Non_coding_probability"` > 0.5 for lncRNA classification. `"Tissue"` for filtering. `"Length"` (INT), `"GC_content"` (REAL).
- **PtRNAdb (ptRNAdb):** Contains plant tRNA gene data across many species. Key columns: `"PtRNAdb ID"`, `"Plant Source"`, `"Genome level"`, `"tRNA ID"`, `"tRNA locus"`, `"Isoacceptor"`, `"Isodecoder"`, `"tRNA Sequence"`, `"tRNA secondary structure"`, `"Upstream sequence (upto 100nt)"`, `"Downstream sequence (upto 100nt)"`, `"Mature tRNA"`, `"Intron Sequence"`, `"Intron Position"`, `"Infernal Score"`, `"HMM Score"`, `"Secondary structure Score"`.
  - Notes: Multi-species (`"Plant Source"`). Scores (`"Infernal Score"`, etc.) are REAL. Use `"Isoacceptor"`, `"Isodecoder"` for tRNA classification.

**CRITICAL DATABASE-SPECIFIC RULES & DATA SEMANTICS:**
- **Cross-Database Joins:**
  - `athisomir` and `anninter2`: Join on `"athisomir"."Sequence" = "anninter2"."interactor1_seq"` (miRNA as interactor1) OR `"athisomir"."Sequence" = "anninter2"."interactor2_seq"` (miRNA as interactor2).
  - `anninter2` and `linc_data`: Join on `"anninter2"."interactor1_seq" = "linc_data"."Sequence"` OR `"anninter2"."interactor2_seq" = "linc_data"."Sequence"`.
  - Example Multi-DB Query ("get miRNA isomiRs from AthisomiRDB and fetch their interactions from ANNInter2"):
    - `sql`: `SELECT "athisomir"."Isomir_ID", "athisomir"."Sequence", "anninter2"."pair_id", "anninter2"."interactor2", "anninter2"."rna_type2", "anninter2"."allenscore" FROM "athisomir" LEFT JOIN "anninter2" ON "athisomir"."Sequence" = "anninter2"."interactor1_seq" OR "athisomir"."Sequence" = "anninter2"."interactor2_seq" WHERE "athisomir"."Isomir_Type" LIKE '%isomiR%' LIMIT {DISPLAY_ROW_LIMIT};`
    - `purpose_type`: `data_preview_multi_db`
    - `target_table`: `athisomir, anninter2`
    - `display_columns_hint`: `["Isomir_ID", "Sequence", "pair_id", "interactor2", "rna_type2", "allenscore"]`
- **Species Handling:**
  - `linc_data`: Use `"Species"` (e.g., `WHERE "Species" LIKE '%Oryza sativa%'` for rice).
  - `ptRNAdb`: Use `"Plant Source"` (e.g., `WHERE "Plant Source" LIKE '%Arabidopsis thaliana%'`).
  - `athisomir`, `anninter2`: Assume Arabidopsis; filter via `"Tissues"` if needed (e.g., `WHERE "Tissues" LIKE '%Arabidopsis%'`).
  - Use `knowledge_graph` hints for common names (e.g., 'rice' -> '%Oryza sativa%').
- **Tissue Handling:**
  - Available in `linc_data.Tissue`, `athisomir.Tissues`. Use `LIKE '%keyword%'` for fuzzy matching (e.g., `WHERE "Tissue" LIKE '%leaf%'`).
  - No tissue column in `anninter2` or `ptRNAdb`.
- **Numeric Columns Stored as TEXT:**
  - `athisomir.RPM`, `anninter2.p_value`, `anninter2.allenscore`, `anninter2.expectation_value` are TEXT; CAST to REAL for comparisons (e.g., `WHERE CAST("RPM" AS REAL) > 10`).
  - `linc_data.Length` (INT), `linc_data.GC_content`, `linc_data.Non_coding_probability` (REAL) are numeric.
  - `ptRNAdb` scores (`Infernal Score`, `HMM Score`, `Secondary structure Score`) are REAL.
- **Expression Data:**
  - Use `athisomir.RPM` for miRNA/isomiR expression (CAST to REAL for aggregations).
  - No direct expression data in `anninter2`, `linc_data`, `ptRNAdb`; use counts for prevalence.
- **Unsupported Molecules:**
  - tRFs, siRNAs, circRNAs are NOT supported. For such queries, set `analysis_plan`: "Query asks about [molecule], which is not covered by these databases." and empty `queries`.
- **Target/Interaction Lookups:**
  - For "miRNAs targeting lncRNAs", query `anninter2` with `WHERE "rna_type1" = 'miRNA' AND "rna_type2" = 'lncRNA' AND "interactor1" LIKE '%query%'`.
  - For reverse lookups (e.g., "lncRNAs targeted by miRNA X"), query `anninter2` with `WHERE "interactor1" LIKE '%X%' AND "rna_type2" = 'lncRNA'`.
- **Fuzzy Matching:**
  - Use `LIKE '%keyword%'` for species, tissues, IDs, sequences to handle misspellings/variations (e.g., 'solanaum' -> `WHERE "Species" LIKE '%Solanum%'`).
- **Vague Follow-ups:**
  - If `user_query` is vague (e.g., 'ok', 'show me', 'yes') and `classified_intent` is `DATA_RETRIEVAL`, recreate the plan based on `previous_query_summary` or `conversation_summary`. Reflect original goal in `analysis_plan`.
- **Comparative Queries:**
  - For "compare X in DB1 vs DB2" (e.g., "compare lncRNA lengths in Alnc vs tRNA lengths in PtRNAdb"), generate separate queries per DB with `purpose_type: aggregation`.
  - For explicit "common" or "intersection" queries (e.g., "common species in Alnc and PtRNAdb"), use `INTERSECT`:
    - `sql`: `SELECT DISTINCT TRIM(LOWER("Species")) AS common_species FROM "linc_data" INTERSECT SELECT DISTINCT TRIM(LOWER("Plant Source")) AS common_species FROM "ptRNAdb";`
    - `purpose_type`: `data_preview_ncRNA_intersection`

**Decision Process & Output Generation:**
1. **Handle Vague Follow-up Commands:**
   - If `user_query` is vague (e.g., 'run it', 'yes') AND `classified_intent` is `DATA_RETRIEVAL`, IGNORE current query text. Recreate plan from `previous_query_summary` or `conversation_summary`. Example: If previous query was "list miRNAs in AthisoMirDB", generate `SELECT * FROM "athisomir" WHERE "Isomir_Type" LIKE '%miRNA%' LIMIT {DISPLAY_ROW_LIMIT};`.

2. **Confirm Intent & Scope:**
   - If `classified_intent` is "OUT_OF_SCOPE", "AMBIGUOUS", "TYPO", or "GENERAL_CONVERSATION", set `analysis_plan` to reflect this and keep `queries` empty.
   - If query mentions unsupported molecules (e.g., tRFs, siRNAs), set `analysis_plan`: "Query asks about [molecule], which is not covered by these databases." and empty `queries`.

3. **Database Targeting Strategy:**
   - **Single DB Queries:**
     - "miRNA"/"isomiR" -> `athisomir` (e.g., `WHERE "Isomir_Type" LIKE '%miRNA%' OR "Isomir_Type" LIKE '%isomiR%'`).
     - "RNA interactions" -> `anninter2` (e.g., `WHERE "rna_type1" = 'miRNA' AND "rna_type2" = 'lncRNA'`).
     - "lncRNA" -> `linc_data` (e.g., `WHERE "Non_coding_probability" > 0.5`).
     - "tRNA"/"transfer RNA" -> `ptRNAdb` (e.g., `WHERE "Isoacceptor" IS NOT NULL`).
   - **Multi-DB Queries:**
     - For queries like "get miRNA isomiRs from AthisomiRDB and fetch their interactions from ANNInter2", use a LEFT JOIN to include all isomiRs and their available interactions:
       - `sql`: `SELECT "athisomir"."Isomir_ID", "athisomir"."Sequence", "athisomir"."RPM", "anninter2"."pair_id", "anninter2"."interactor2", "anninter2"."rna_type2" FROM "athisomir" LEFT JOIN "anninter2" ON "athisomir"."Sequence" = "anninter2"."interactor1_seq" OR "athisomir"."Sequence" = "anninter2"."interactor2_seq" WHERE "athisomir"."Isomir_Type" LIKE '%isomiR%' LIMIT {DISPLAY_ROW_LIMIT};`
       - `purpose_type`: `data_preview_multi_db`
       - `target_table`: `athisomir, anninter2`
   - **Comparative Queries:**
     - Generate separate queries for each DB. Example: "Compare lncRNA lengths in Alnc vs tRNA lengths in PtRNAdb for rice":
       - Query 1: `SELECT AVG("Length") AS avg_length FROM "linc_data" WHERE "Species" LIKE '%Oryza sativa%';`
       - Query 2: `SELECT AVG(LENGTH("tRNA Sequence")) AS avg_length FROM "ptRNAdb" WHERE "Plant Source" LIKE '%Oryza sativa%';`
       - `purpose_type`: `aggregation`
   - **Species Handling:** Use `knowledge_graph` for mappings (e.g., 'cotton' -> '%Gossypium%'). For Arabidopsis, no species filter for `athisomir`/`anninter2`.

4. **Response Strategy & SQL Planning:**
   - **METADATA_DIRECT_ANSWER_PREFERRED (e.g., "describe AthisoMirDB", "what are isomiRs?"):**
     ...
     - Populate `direct_answer_from_metadata` with a comprehensive answer from `retrieved_knowledge_context`.
     - `queries`: Empty.
     - Exception: If query is "how many species in linc_data?" and context lacks answer, set `query_type`: `metadata_sql_fallback`, `analysis_plan`: "Counting distinct {{item_type}}...", and generate SQL: `SELECT COUNT(DISTINCT "Species") FROM "linc_data";`.
   - **METADATA_SQL_FALLBACK_PREFERRED (e.g., "list tissues in Alnc", "species in PtRNAdb"):**
     - `query_type`: `metadata_sql_fallback`
     - `analysis_plan`: "Listing distinct {{item_type}} from {{database_conceptual_name}}."
     - `purpose_type`: `list_distinct_values_for_common_check_tissue` (or `_species`)
     - `sql`: `SELECT DISTINCT "Tissue" AS item_value FROM "linc_data" WHERE "Tissue" IS NOT NULL AND TRIM("Tissue") != '' AND LOWER(TRIM("Tissue")) NOT IN ('na', 'unknown', 'none') ORDER BY 1;`
     - For multiple DBs (e.g., "common species in Alnc and PtRNAdb"):
       - `sql`: `SELECT DISTINCT TRIM(LOWER("Species")) AS common_species FROM "linc_data" INTERSECT SELECT DISTINCT TRIM(LOWER("Plant Source")) AS common_species FROM "ptRNAdb";`
       - `purpose_type`: `data_preview_ncRNA_intersection`
   - **DATA_RETRIEVAL:**
     - **Simple Listings (e.g., "list tRNAs in rice"):**
       - `purpose_type`: `data_preview`
       - `sql`: `SELECT "PtRNAdb ID", "tRNA ID", "Isoacceptor", "tRNA Sequence" FROM "ptRNAdb" WHERE "Plant Source" LIKE '%Oryza sativa%' LIMIT {DISPLAY_ROW_LIMIT};`
       - `display_columns_hint`: `["PtRNAdb ID", "tRNA ID", "Isoacceptor"]`
     - **Entity Lookups (e.g., "details for Isomir_ID ATHISO123"):**
       - `purpose_type`: `entity_lookup_detail`
       - `sql`: `SELECT * FROM "athisomir" WHERE "Isomir_ID" = 'ATHISO123';`
       - `display_columns_hint`: All relevant columns (e.g., `["Isomir_ID", "Sequence", "RPM", "Tissues"]`)
     - **Aggregations (e.g., "count tRNAs in rice"):**
       - `purpose_type`: `aggregation_count`
       - `sql`: `SELECT COUNT(*) AS count FROM "ptRNAdb" WHERE "Plant Source" LIKE '%Oryza sativa%';`
       - `display_columns_hint`: `["count"]`
     - **Comparative Queries (e.g., "compare miRNA expression in leaf vs root"):**
       - `purpose_type`: `aggregation`
       - Query 1: `SELECT "Tissues", AVG(CAST("RPM" AS REAL)) AS avg_rpm FROM "athisomir" WHERE "Tissues" LIKE '%leaf%' GROUP BY "Tissues";`
       - Query 2: `SELECT "Tissues", AVG(CAST("RPM" AS REAL)) AS avg_rpm FROM "athisomir" WHERE "Tissues" LIKE '%root%' GROUP BY "Tissues";`
       - `display_columns_hint`: `["Tissues", "avg_rpm"]`
     - **Multi-DB Queries (e.g., "get miRNA isomiRs and their interactions"):**
       - `purpose_type`: `data_preview_multi_db`
       - Use JOIN or CTE to combine data; prioritize LEFT JOIN for optional matches.
       - Example: `SELECT "athisomir"."Isomir_ID", "athisomir"."RPM", "anninter2"."pair_id", "anninter2"."interactor2" FROM "athisomir" LEFT JOIN "anninter2" ON "athisomir"."Sequence" = "anninter2"."interactor1_seq" OR "athisomir"."Sequence" = "anninter2"."interactor2_seq" WHERE "athisomir"."Isomir_Type" LIKE '%isomiR%' LIMIT {DISPLAY_ROW_LIMIT};`
   - **Vague Data Requests (e.g., "what data on cotton?"):**
     - Interpret as `data_preview`.
     - Select relevant DB (e.g., `linc_data` for cotton -> `WHERE "Species" LIKE '%Gossypium%'`).
     - `sql`: `SELECT * FROM "linc_data" WHERE "Species" LIKE '%Gossypium%' LIMIT {DISPLAY_ROW_LIMIT};`

5. **SQL Best Practices:**
   - Always quote identifiers: `"Table Name"`, `"Column Name"`.
   - Use `TRIM(LOWER())` for case-insensitive comparisons in DISTINCT/INTERSECT.
   - Apply `LIMIT {DISPLAY_ROW_LIMIT}` for preview queries.
   - For numeric filters, CAST TEXT columns: `WHERE CAST("allenscore" AS REAL) > 0.5`.

6. **Handling Complex Concepts:**
   - For queries like "miRNAs with high expression", use `athisomir.RPM` with `CAST("RPM" AS REAL) > threshold`.
   - For interactions, check `anninter2.rna_type1`/`rna_type2` and use `p_value`/`allenscore` for significance.

7. **Common Item Analysis:**
   - Trigger: Query uses "common", "shared", "intersection", "both", "compare", "vs".
   - Action: Generate `INTERSECT` query for exact matches:
     - Example: "common species in Alnc and PtRNAdb":
       - `sql`: `SELECT DISTINCT TRIM(LOWER("Species")) AS common_species FROM "linc_data" INTERSECT SELECT DISTINCT TRIM(LOWER("Plant Source")) AS common_species FROM "ptRNAdb";`
       - `purpose_type`: `data_preview_ncRNA_intersection`
       - `display_columns_hint`: `["common_species"]`
...
{{
  ...
  "queries": [
    {{
      ...
      "target_table": "table_name OR table1, table2",
      "database_conceptual_name": "Conceptual Name(s)",
      "description": "Goal of this SQL query.",
      "purpose_type": "data_preview|entity_lookup_detail|aggregation|aggregation_count|list_distinct_values_for_common_check_species|list_distinct_values_for_common_check_tissue|data_preview_multi_db|data_preview_ncRNA_intersection",
...
*** You are a specialized chatbot for answering queries related to miRNA, isomiRs, lncRNAs, tRNAs, and RNA-RNA interactions, catering to the following databases: AthisoMir Database, AnnInter2 Database, Alnc Database, and PtRNAdb. ***
...
- **AthisoMir Database (athisomir)**: A comprehensive repository of Arabidopsis thaliana isomiRs, containing over 20,764 isoforms. Key data includes `Isomir_ID`, `Sequence`, `RPM` (expression), and `Parent` miRNA.
- **AnnInter2 Database (anninter2)**: A collection of ncRNA-ncRNA interactions in Arabidopsis thaliana, including miRNA-lncRNA interactions. Key data includes `pair_id`, `interactor1_seq`, `interactor2_seq`, `p_value`, and `allenscore`.
...
- **PtRNAdb (PtRNAdb)**: A plant-exclusive database of tRNA genes from 106 nuclear, 89 plastidial, and 38 mitochondrial genomes. Key data includes `PtRNAdb ID`, `Plant Source` (species), `Genome level`, `tRNA ID`, `Isoacceptor`, `tRNA Sequence`, and prediction scores.
...
- Use conceptual database names (e.g., PtRNAdb) instead of internal table names.
...
- For lists of items (e.g., species from `PtRNAdb`), present them clearly.
...
- List unique conceptual database names (e.g., "PtRNAdb") explicitly mentioned in the "System-Generated Data Summary".
...
{{
  ...
  "databases_conceptually_involved": ["AthisoMir Database", "PtRNAdb"]
}}
...
def process_query(query: str, conversation_id: Optional[str] = None, model_name: str = 'gpt-oss-20b', user_api_key: Optional[str] = None) -> Dict:
    ...
    - AthisoMir Database: Contains Arabidopsis thaliana isomiRs (miRNA variants). Key columns: `Isomir_ID`, `Sequence`, `RPM` (expression).
    - AnnInter2 Database: Contains ncRNA-ncRNA interactions in Arabidopsis thaliana. Key columns: `pair_id`, `interactor1_seq`, `interactor2_seq`, `p_value`.
    - Alnc Database: Contains lncRNAs across 682 plant species. Key columns: `AlnC_ID`, `Species`, `Tissue`, `Non_coding_probability`.
    - PtRNAdb: Contains tRNA genes across 106+ plant species. Key columns: `PtRNAdb ID`, `Plant Source`, `tRNA ID`, `Isoacceptor`.
    ...
    species_info_str = "\n".join(species_info_parts) if species_info_parts else "No specific species data pre-loaded. AthisoMir and AnnInter2 are Arabidopsis thaliana only."
...
- Tables & Primary Use (Conceptual Names from DATABASE_MAPPING):
  - {DATABASE_MAPPING.get("athisomir", "AthisoMir Database")}: IsomiR (miRNA variants) data for Arabidopsis thaliana. Species: Implicitly Arabidopsis thaliana. Key cols: `Isomir_ID`, `Sequence`, `RPM`.
  - {DATABASE_MAPPING.get("anninter2", "AnnInter2 Database")}: RNA-RNA interactions. Species: Implicitly Arabidopsis thaliana. Key cols: `pair_id`, `interactor1_seq`, `rna_type1`, `p_value`.
  ...
  - {DATABASE_MAPPING.get("PtRNAdb", "PtRNAdb")}: tRNA genes across many plant species. Species col: `Plant Source`. No tissue data. Key cols: `PtRNAdb ID`, `tRNA ID`, `Isoacceptor`, `tRNA Sequence`.

- **Handling of Complex Concepts:**
  - **miRNA/isomiR Queries:** Use `athisomir`.
  - **RNA Interactions:** Use `anninter2`.
  - **lncRNA Queries:** Use `linc_data`.
  - **tRNA Queries:** Use `PtRNAdb`.
...
- **Default Database Grouping for General Queries:**
  - "miRNA" or "isomiR" queries: Use `athisomir`.
  - "RNA interaction" queries: Use `anninter2`.
  - "lncRNA" queries: Use `linc_data`.
  - "tRNA" or "transfer RNA" queries: Use `PtRNAdb`.

- **Categories for Common Item Checks:**
  - "species": linc_data (`Species`); PtRNAdb (`Plant Source`); athisomir, anninter2 (implicit Arabidopsis thaliana)
    ...
    if query_type_from_stage1 == "metadata_answered_from_blob":
        ...
        if direct_answer_from_metadata and direct_answer_from_metadata.strip():
            ...
            concept_keywords_map = {
                "mirna": ["miRNA (microRNA)", "AthisoMir Database"],
                "isomir": ["miRNA (microRNA)", "AthisoMir Database"],
                "lncrna": ["lncRNA (long non-coding RNA)", "Alnc Database"],
                "interaction": ["RNA Interactions", "AnnInter2 Database"]
            }
            ...
            DATABASE_METADATA_KNOWLEDGE_BLOB = """
*   **AthisoMir Database (athisomir):** A comprehensive database of Arabidopsis thaliana isomiRs, curated from 2,757 miRNA-seq samples, containing over 20,764 isoforms. Key data: `Isomir_ID`, `Sequence`, `Isomir_Type`, `RPM`, `Parent`.
*   **AnnInter2 Database (anninter2):** Contains ncRNA-ncRNA interactions in Arabidopsis thaliana, predicted and validated with degradome-seq. Key data: `pair_id`, `interactor1_seq`, `interactor2_seq`, `rna_type1`, `rna_type2`, `p_value`, `allenscore`.
