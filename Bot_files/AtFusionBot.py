...
API_KEY = os.getenv('GROQ_API_KEY', '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')
        ...
        self.schema = {
            'all_databases.db': {
                'tables': {
                    'AtFusionDB': {
                        'columns': [
                            {'name': 'identifier', 'sqlite_type': 'TEXT', 'semantic_type': 'primary_identifier', 'description': 'Unique identifier for the fusion event.'},
                            {'name': 'fusion_name', 'sqlite_type': 'TEXT', 'semantic_type': 'name', 'description': 'Name of the fusion transcript.'},
                            {'name': 'gene1', 'sqlite_type': 'TEXT', 'semantic_type': 'gene_identifier', 'description': 'First gene involved in the fusion.'},
                            {'name': 'gene2', 'sqlite_type': 'TEXT', 'semantic_type': 'gene_identifier', 'description': 'Second gene involved in the fusion.'},
                            {'name': 'position1', 'sqlite_type': 'TEXT', 'semantic_type': 'genomic_position', 'description': 'Genomic position of gene1.'},
                            {'name': 'position2', 'sqlite_type': 'TEXT', 'semantic_type': 'genomic_position', 'description': 'Genomic position of gene2.'},
                            {'name': 'junction_sequence', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'Sequence at fusion junction.'},
                            {'name': 'tissue', 'sqlite_type': 'TEXT', 'semantic_type': 'tissue_type', 'description': 'Tissue where fusion was detected.'},
                            {'name': 'fusion_type', 'sqlite_type': 'TEXT', 'semantic_type': 'category', 'description': 'Type of fusion event.'},
                            {'name': 'condition', 'sqlite_type': 'TEXT', 'semantic_type': 'experimental_condition', 'description': 'Experimental condition.'},
                            {'name': 'srr_id', 'sqlite_type': 'TEXT', 'semantic_type': 'accession_id', 'description': 'Sequence Read Archive ID.'},
                            {'name': 'source_table', 'sqlite_type': 'TEXT', 'semantic_type': 'metadata', 'description': 'Source table for data.'},
                            {'name': 'sequence', 'sqlite_type': 'TEXT', 'semantic_type': 'biological_sequence', 'description': 'Full sequence, may contain "unable to predict".'}
                        ],
                        'description': 'Stores fusion transcripts in Arabidopsis thaliana with gene pairs, tissue, and sequence data.',
                        'primary_keys': ['identifier'],
                        'common_joins': {'PFusionDB': "Potentially on `gene1`, `gene2`, or implicit 'Arabidopsis thaliana' species."},
                        'notes': ["Data specific to Arabidopsis thaliana. No 'species' or 'organism' column exists."]
                    },
                }
            }
        }
...
*** You are only speciealised chatbot for answering fusion transcript related querries and cater the following databases on fusion transcripts that are AtFusionDB Database and
 PFusionDB Database. ***
...
*** You are only speciealised chatbot for answering fusion transcript related querries and cater the following database on fusion transcripts that is AtFusionDB Database. ***
...
2.  **Database Targeting Strategy (Guided by `knowledge_graph` and above CRITICAL RULES):**
    *   **Default Grouping (if no specific DB named by user):** Use groupings specified in `knowledge_graph`.
        *   Example: "fusions" -> AtFusionDB (implicitly Arabidopsis).
    *   **Species Name Reasoning:** Use hints from `knowledge_graph` (e.g., "rice" -> `LIKE 'Oryza sativa%'`, "cotton" -> `LIKE 'Gossypium%'`). For "Arabidopsis", target AtFusionDB (no species filter needed).
...
3.  **Response Strategy & SQL Planning:**
    ...
    *   **IF {classified_intent} is "METADATA_DIRECT_ANSWER_PREFERRED" (e.g., "describe [DB_name]", "what is [concept]?"):**
        ...
        **CRITICAL: `direct_answer_from_metadata` field MUST be populated with a comprehensive answer synthesized from `retrieved_knowledge_context`.** 
...
5.  **ALWAYS Quote String Values:** All string literals used for comparison in a `WHERE` clause **MUST** be enclosed in single quotes (`'`). Wildcards for a `LIKE` clause must be *inside* these single quotes.
    - **CORRECT:** `WHERE "species" LIKE 'Arabidopsis thaliana%'`
    - **INCORRECT:** `WHERE "species" LIKE 'Arabidopsis thaliana'%`


7.  **Handling Vague Data Requests (e.g., "what data on cotton?"):**
    - If a user asks a "what data do you have on [topic]?" or "show me data about [topic]" question, interpret this as a `data_preview` request.
...
8.  **Fuzzy Matching and Name Variations:**
...
13.  **Common Item Analysis:**
  ...
  **Quote String Values (in WHERE clauses):** Always use single quotes (`'`). Wildcards (`%`) for `LIKE` clauses **MUST** be placed *inside* the single quotes. This is a mandatory rule. Failure to follow it results in a syntax error.
    *   **CORRECT:** `WHERE "species" LIKE 'Arabidopsis thaliana%'`
    *   **CORRECT:** `WHERE "species" LIKE 'Oryza sativa%'`
    *   **INCORRECT:** `WHERE "species" LIKE 'Arabidopsis thaliana'%`  <-- DO NOT PRODUCE THIS.
    *   **INCORRECT:** `WHERE "species" = "Arabidopsis thaliana"` <-- DO NOT USE DOUBLE QUOTES FOR VALUES.

    **Database Targeting Strategy:**
    *   For "fusion" queries, you must plan to query **BOTH** `AtFusionDB` and `PFusionDB`.
    *   **`AtFusionDB` Rule:** This table is implicitly `Arabidopsis thaliana`. **DO NOT** add a `WHERE "species" ...` clause to its queries.
    *   **`PFusionDB` Rule:** This table is multi-species. If the user's query implies a species (e.g., an 'AT' gene for Arabidopsis, an 'Os' gene for Rice), you **MUST** add a `WHERE "species" LIKE 'species_name%'` clause, following the CRITICAL SYNTAX RULES above.

    **Plan Generation:**
    *   If the query is for a gene (e.g., "do AT1G65440 make fusions?"), your `analysis_plan` must state that you will check both fusion databases.
    *   Generate two separate query objects in the `queries` array.
        *   **Query 1 (AtFusionDB):** `purpose_type`: `entity_lookup_detail`. `sql`: `SELECT * FROM "AtFusionDB" WHERE "gene1" = 'AT1G65440' OR "gene2" = 'AT1G65440';`
        *   **Query 2 (PFusionDB):** `purpose_type`: `entity_lookup_detail`. `sql`: `SELECT * FROM "PFusionDB" WHERE ("Gene1" = 'AT1G65440' OR "Gene2" = 'AT1G65440') AND "species" LIKE 'Arabidopsis thaliana%';` (Ensure this SQL is perfectly formed per the rules).
 
15.  **JSON Output Structure (MUST be followed precisely):**
    <JSON_START>
    {{
...
*** You are only speciealised chatbot for answering fusion transcript related querries and cater the following databases on fusion transcripts that is AtFusionDB Database. ***
...
8.  **TASK: HANDLING DISTINCT LISTS (e.g., "what species in AtFUsiondb"):**
