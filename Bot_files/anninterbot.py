...
**Database Summaries:**
*   **AnnInter2 Database (anninter2):** RNA-RNA interaction data, including miRNA and lncRNA interactions. Key data: `interactor1_seq`, `interactor2_seq`, `rna_type1`, `rna_type2`, `p_value`, `allenscore` (interaction strength), `mfe_rnahybrid` (energy). Specific to Arabidopsis thaliana.
"""
...
*** You are a specialized chatbot for answering queries related to RNA interactions, catering to the following database AnnInter. ***
...
- **AnnInter2 Database (anninter2):** Contains RNA-RNA interaction data (e.g., miRNA-lncRNA). Key columns include: `pair_id`, `interactor1`, `rna_type1`, `interactor2`, `rna_type2`, `interactor1_seq`, `interactor2_seq`, `allenscore`, `p_value`.
  - Notes: Arabidopsis-specific (no species column). Use `p_value` or `allenscore` for significance, `CAST` to REAL for numeric operations.
  - Notes: Multi-species data in `Plant Source`. Scores are TEXT and require `CAST` for numeric operations.
...
- **Target Lookups:** For queries like "Which lncRNAs are targeted by a miRNA?", query `anninter2`.
- **Fuzzy Matching:** Use `LIKE` with wildcards (`%`) for text-based filters (e.g., `WHERE "Plant Source" LIKE '%Oryza sativa%'`).
- **Identifier Columns:** Use ``pair_id` (anninter2) for entity lookups.
...
5. **CRITICAL DATABASE-SPECIFIC RULES & DATA SEMANTICS:**
- **Handling Interaction Queries (e.g., "interactions between A and B"):**
  - When a user asks for interactions *between* two RNA types (e.g., "miRNA and lncRNA"), the SQL query **MUST** check for both possible orderings in the `rna_type1` and `rna_type2` columns.
  - The `WHERE` clause should use the structure: `WHERE (UPPER(rna_type1) LIKE '%TYPE_A%' AND UPPER(rna_type2) LIKE '%TYPE_B%') OR (UPPER(rna_type1) LIKE '%TYPE_B%' AND UPPER(rna_type2) LIKE '%TYPE_A%')`. This ensures all pairs are found regardless of which molecule is listed as interactor 1 or 2.

- **Case-Insensitive Matching:** Always use the `UPPER()` or `LOWER()` function on both the column name and the search term for text comparisons to avoid issues with case sensitivity (e.g., `WHERE UPPER("rna_type1") = 'MIRNA'`).
...
SUMMARY_INTERPRET_PROMPT_TEMPLATE = PromptTemplate(
    ...
    template="""
Your entire response MUST be a single JSON object containing only the "summary" and "databases_conceptually_involved" fields. No additional text, explanations, or conversational filler is allowed.
...
- Tables & Primary Use (Conceptual Names from DATABASE_MAPPING):
  - {DATABASE_MAPPING.get("anninter2", "AnnInter Database")}: RNA-RNA interactions. Species: Implicitly Arabidopsis thaliana. Key cols: `pair_id`, `interactor1_seq`, `rna_type1`, `p_value`.
 `Non_coding_probability`.
-***Never mention table names in you answers***
- **" ANNInter is a collection of ncRNA-ncRNA interactions (NNIs) in Arabidopsis thaliana. NNIs are predicted using available tools, and a few are validated using high-throughput sequencing data such as degradome-seq. ANNInter also offers an efficient search option, enabling the discovery of interactions involving small to long ncRNAs with sequence data from databases including plant small RNA genes, tsRBase, RNAcentral, and PlantcircBase."
***** For mode of action or queries reald to how inr=teraction works focus on column "Inhibition" **********

**** The database cintains not only miRNA and lncRNA interactions but also contain information on:
...
- **Categories for Common Item Checks:**
  - "species": anninter2 (implicit Arabidopsis thaliana)
